...
--CRIAǃO DA TABELA MUNICIPIO (INICIO)
...
--CRIAǃO DA TABELA MUNICIPIO (FIM)
...
--CRIAǁO DA TABELA ORDEMDETRANSPORTE (INICIO)
...
--CRIAǁO DA TABELA ORDEMDETRANSPORTE (FIM)

--ALTERAǃO DA TABELA FORNECEDOR (INICIO)
ALTER TABLE FORNECEDOR 
ADD (ENDERECO VARCHAR2(250 CHAR) );
--ALTERAǃO DA TABELA FORNECEDOR (FIM)	  
...
--ALTERAǃO NA VIEW AGENDAMENTODECARGAVISUALIZACAO (INICIO)
...
--ALTERAǃO NA VIEW AGENDAMENTODECARGAVISUALIZACAO (FIM)
...
--INCLUSÏ DE NOVOS CAMPOS NA TABELA AGENDAMENTODECARREGAMENTO (INICIO)
ALTER TABLE AGENDAMENTODECARREGAMENTO 
ADD (MOTORISTA VARCHAR2(100 CHAR) );

ALTER TABLE AGENDAMENTODECARREGAMENTO 
ADD (DESTINO VARCHAR2(100 CHAR) );
--INCLUSÏ DE NOVOS CAMPOS NA TABELA AGENDAMENTODECARREGAMENTO (FIM)





--INCLUSÏ DO NڍERO DA CONDIǃO NA COTACAO DE FRETE (INICIO)
ALTER TABLE COTACAOFRETE 
ADD (NUMERODACONDICAOGERADANOSAP VARCHAR2(10 CHAR) );
--INCLUSÏ DO NڍERO DA CONDIǃO NA COTACAO DE FRETE (FIM)

--GERADO VERSÏ EM PRODUǃO 31/05/2014

--INICIO DAS ALTERAǕES (MATERIAL DE CARGA)
...
--GERADO VERSÏ EM PRODUǃO EM 09/06/2014
...
--SE NÏ TEM COTACAO INICIA COMO PENDENTE. SE TEM COTAǁO INICIA COMO ACEITO
...
---alterações 06/06/2015 (fim)

--alterações 08/06/2015 (inicio)
...
--alterações 08/06/2015 (fim)

--script para passar conhecimentos para o status "SEM OT"
UPDATE CONHECIMENTODETRANSPORTE SET STATUS = 3 -- SEM OT
WHERE STATUS = 0 -- NAO ATRIBUIDO
AND NOT EXISTS 
(
	SELECT 1  FROM CONHECIMENTO_ORDEMTRANSPORTE CTOT 
	WHERE CONHECIMENTODETRANSPORTE.CHAVEELETRONICA = CTOT.CHAVEELETRONICA
);
...
--CRIAÇÃ DA COLUNA DataFechamento na tabela ProcessoCotacao (INICIO)
...
--INSERE HISTÓICO PARA O PREÇO FINAL, CASO SEJA DIFERENTE DO PREÇO INICIAL
INSERT INTO HISTORICODEPRECO
(ID, IDCOTACAOITEM, DATAHORA, VALOR)
...
(
  ...
  DESCRICAO VARCHAR2(80 CHAR) NOT NULL,
CONSTRAINT PK_COTACAOHISTORICO PRIMARY KEY (ID) ENABLE
);

CREATE SEQUENCE COTACAO_HISTORICO_ID_SEQUENCE INCREMENT BY 1 START WITH 1;

CREATE INDEX IDX_COTACAOHIST_FORNECPARC ON COTACAOHISTORICO(IDFORNECEDORPARTICIPANTE);
