...
--ALTERA«√O NA VIEW AGENDAMENTODECARGAVISUALIZACAO (INICIO)
...
--ALTERA«√O NA VIEW AGENDAMENTODECARGAVISUALIZACAO (FIM)
...
--INCLUS√O DE NOVOS CAMPOS NA TABELA AGENDAMENTODECARREGAMENTO (INICIO)
ALTER TABLE AGENDAMENTODECARREGAMENTO 
ADD (MOTORISTA VARCHAR2(100 CHAR) );

ALTER TABLE AGENDAMENTODECARREGAMENTO 
ADD (DESTINO VARCHAR2(100 CHAR) );
--INCLUS√O DE NOVOS CAMPOS NA TABELA AGENDAMENTODECARREGAMENTO (FIM)





--INCLUS√O DO N⁄MERO DA CONDI«√O NA COTACAO DE FRETE (INICIO)
ALTER TABLE COTACAOFRETE 
ADD (NUMERODACONDICAOGERADANOSAP VARCHAR2(10 CHAR) );
--INCLUS√O DO N⁄MERO DA CONDI«√O NA COTACAO DE FRETE (FIM)

--GERADO VERS√O EM PRODU«√O 31/05/2014

--INICIO DAS ALTERA«’ES (MATERIAL DE CARGA)
...
--GERADO VERS√O EM PRODU«√O EM 09/06/2014
...
--SE N√O TEM COTACAO INICIA COMO PENDENTE. SE TEM COTA«AO INICIA COMO ACEITO
...
BEGIN
  ...
  ON OT.ID = CR.IDORDEMTRANSPORTE
  LEFT JOIN (
    SELECT 0 AS ID, 'Negocia√ß√£o de Tarifa' AS DESCRICAO FROM DUAL
    UNION 
    SELECT 1 AS ID,'N√£o cumprimento do Contrato' AS DESCRICAO FROM DUAL 
    UNION 
    SELECT 2 AS ID,'Altera√ß√£o de local de coleta' AS DESCRICAO FROM DUAL
...
ALTER TABLE CONHECIMENTODETRANSPORTE MODIFY NUMERODOCONTRATO VARCHAR2(20 CHAR)NULL; 
...
--DADOS DA COLETA
SELECT 'COLETA' || IDORDEMTRANSPORTE || C.ID || NFC.NUMERO AS ID, NULL AS IDQUOTA, NULL AS IDAGENDAMENTO, IDORDEMTRANSPORTE,C.ID AS IDCOLETA, P.DESCRICAO AS  DESCRICAOMATERIAL, 
2  AS CODIGOFLUXO, 'Descarregamento' AS DESCRICAOFLUXO, TO_DATE(TO_CHAR(C.DATADEPREVISAODECHEGADA, 'dd/mm/yyyy')) AS DATAAGENDAMENTO, C.PLACA, NFC.NUMERO AS NUMERONF,
FM.CNPJ AS CNPJEMITENTE, FM.NOME AS NOMEEMITENTE, C.REALIZADO,T.CODIGO AS CODIGOTERMINAL, T.NOME AS DESCRICAOTERMINAL,PF.CODIGODEPOSITO
...
---altera√ß√µes 06/06/2015 (inicio)

ALTER TABLE ORDEMDETRANSPORTE ADD VALORPLANEJADO NUMBER(13,2);
ALTER TABLE ORDEMDETRANSPORTE ADD VALORREAL NUMBER(13,2);

UPDATE ORDEMDETRANSPORTE SET 
VALORPLANEJADO = quantidadecoletada * precounitario,
VALORREAL = nvl((select sum(valordofrete) from coleta where ordemdetransporte.id = coleta.idordemtransporte),0);

COMMIT;

ALTER TABLE ORDEMDETRANSPORTE MODIFY VALORPLANEJADO NUMBER(13,2) NOT NULL;
ALTER TABLE ORDEMDETRANSPORTE MODIFY VALORREAL NUMBER(13,2) NOT NULL;
...
BEGIN
  ...
  CAST(OT.QUANTIDADEREALIZADA AS DECIMAL) AS "QuantidadeRealizada", OT.QuantidadeLiberada - OT.QuantidadeRealizada AS "QuantidadePendente", UM.DESCRICAO AS "UnidadeDeMedida",
  MOTIVODEFECHAMENTO.DESCRICAO AS "MotivoDeFechamento", OT.ObservacaoDeFechamento AS "ObservacaoDeFechamento", 
  CAST(OT.VALORPLANEJADO AS DECIMAL) AS "ValorPlanejado", CAST(OT.VALORREAL AS DECIMAL) AS "ValorReal"
  FROM ORDEMDETRANSPORTE OT INNER JOIN PROCESSOCOTACAOFRETE PCF ON OT.IDPROCESSOCOTACAOFRETE = PCF.ID
  INNER JOIN PROCESSOCOTACAO PC ON PCF.ID = PC.ID
  INNER JOIN PRODUTO P ON PC.CODIGOPRODUTO = P.CODIGO
  INNER JOIN UNIDADEMEDIDA UM ON PC.CODIGOUNIDADEMEDIDA = UM.CODIGOINTERNO
...
---altera√ß√µes 06/06/2015 (fim)

--altera√ß√µes 08/06/2015 (inicio)
ALTER TABLE ORDEMDETRANSPORTE DROP COLUMN VALORREAL;
ALTER TABLE ORDEMDETRANSPORTE ADD CHAVECONHECIMENTOTRANSPORTE VARCHAR2(44 BYTE);
ALTER TABLE ORDEMDETRANSPORTE ADD CONSTRAINT FK_ORDEM_CONHECIMENTO FOREIGN KEY  (CHAVECONHECIMENTOTRANSPORTE) REFERENCES CONHECIMENTODETRANSPORTE(CHAVEELETRONICA);

UPDATE ORDEMDETRANSPORTE SET
CHAVECONHECIMENTOTRANSPORTE = 
(
	SELECT CT.CHAVEELETRONICA FROM CONHECIMENTODETRANSPORTE CT INNER JOIN CONHECIMENTO_ORDEMTRANSPORTE CTOT
	ON CT.CHAVEELETRONICA = CTOT.CHAVEELETRONICA
	WHERE ORDEMDENTRASPORTE.ID = CTOT.IDORDEMTRANSPORTE
	AND CT.STATUS = 1
);
...
BEGIN
  ...
  CAST(OT.QUANTIDADEREALIZADA AS DECIMAL) AS "QuantidadeRealizada", OT.QuantidadeLiberada - OT.QuantidadeRealizada AS "QuantidadePendente", UM.DESCRICAO AS "UnidadeDeMedida",
  MOTIVODEFECHAMENTO.DESCRICAO AS "MotivoDeFechamento", OT.ObservacaoDeFechamento AS "ObservacaoDeFechamento", 
  CAST(OT.VALORPLANEJADO AS DECIMAL) AS "ValorPlanejado", CAST(NVL(CT.ValorRealDoFrete,0) AS DECIMAL) AS "ValorReal"
  FROM ORDEMDETRANSPORTE OT INNER JOIN PROCESSOCOTACAOFRETE PCF ON OT.IDPROCESSOCOTACAOFRETE = PCF.ID
  INNER JOIN PROCESSOCOTACAO PC ON PCF.ID = PC.ID
  INNER JOIN PRODUTO P ON PC.CODIGOPRODUTO = P.CODIGO
  INNER JOIN UNIDADEMEDIDA UM ON PC.CODIGOUNIDADEMEDIDA = UM.CODIGOINTERNO
  ...
  INNER JOIN FORNECEDOR T ON OT.CODIGOFORNECEDOR = T.CODIGO
  INNER JOIN TERMINAL ON PCF.CODIGOTERMINAL = TERMINAL.CODIGO
  LEFT JOIN CONHECIMENTODETRANSPORTE CT ON OT.CHAVECONHECIMENTOTRANSPORTE = CT.CHAVEELETRONICA
  LEFT JOIN FORNECEDOR D ON PCF.CODIGODEPOSITO = D.CODIGO
  LEFT JOIN (
    SELECT IDORDEMTRANSPORTE, COUNT(1) AS QuantidadeDeColetasRealizadas, 
    SUM(CASE WHEN DATADECHEGADA > DATADEPREVISAODECHEGADA THEN DATADECHEGADA - DATADEPREVISAODECHEGADA ELSE 0 END) AS QuantidadeDeDiasEmAtraso,
